...
# Esempio di utilizzo (estratto dal main)
if __name__ == '__main__':
    Q = QLearningAgent(0.5, 0.9, 1, 1)
    games = 25000
    ...
    for i in tqdm(range(games)):
        ...
        while game.check_winner() == -1:
            ...
            state = game.get_board()
            actions = game.possible_moves(player)
            action = Q.choose_action(state, actions)
            game.move(action[0], action[1], player)
            trajectory.append((state, action))

            player = 1 - player

        Q.update_q_table(trajectory, Q.reward(game.check_winner()))
        trajectory = []

    with open('Q_table1.txt', 'w') as f:
